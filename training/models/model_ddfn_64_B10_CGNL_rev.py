...
class MsFeat(nn.Module):
    def __init__(self, in_channels):
        ...
        self.ds = nn.Sequential(nn.Conv3d(in_channels, in_channels, 7, stride=(2,1,1), padding=3, dilation=1, groups=1, bias=True), nn.ReLU(inplace=True))
        init.kaiming_normal_(self.ds[0].weight, 0, 'fan_in', 'relu'); init.constant_(self.ds[0].bias, 0.0)

        self.conv1 = nn.Sequential(nn.Conv3d(in_channels, outchannel_MS, 3, stride=(2,1,1), padding=1, dilation=1, bias=True), nn.ReLU(inplace=True))
        init.kaiming_normal_(self.conv1[0].weight, 0, 'fan_in', 'relu'); init.constant_(self.conv1[0].bias, 0.0)

        self.conv2 = nn.Sequential(nn.Conv3d(in_channels, outchannel_MS, 3, stride=(2,1,1), padding=2, dilation=2, bias=True), nn.ReLU(inplace=True))
...
class CGNL(nn.Module):
    def __init__(self, inplanes, use_scale=False, groups=None):
        ...
        # conv theta
        self.t = nn.Conv3d(inplanes, inplanes//4, kernel_size=1, stride=1, bias=False)
        init.kaiming_normal_(self.t.weight, 0, 'fan_in', 'relu')
        # conv phi
        self.p = nn.Conv3d(inplanes, inplanes//4, kernel_size=1, stride=1, bias=False)
        init.kaiming_normal_(self.p.weight, 0, 'fan_in', 'relu')
        # conv g
        self.g = nn.Conv3d(inplanes, inplanes//4, kernel_size=1, stride=1, bias=False)
        init.kaiming_normal_(self.g.weight, 0, 'fan_in', 'relu')
        # conv z
        self.z = nn.Conv3d(inplanes//4, inplanes, kernel_size=1, stride=1,
...
class DeepBoosting(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.ds1 = nn.Sequential(nn.Conv3d(32,32,kernel_size=5,stride=(2,1,1),padding=(2,2,2),bias=True),nn.ReLU(inplace=True))
        init.kaiming_normal_(self.ds1[0].weight, 0, 'fan_in', 'relu'); init.constant_(self.ds1[0].bias, 0.0)
        self.ds2 = nn.Sequential(nn.Conv3d(32, 32, kernel_size=5, stride=(2, 1, 1), padding=(2, 2, 2), bias=True),nn.ReLU(inplace=True))
        init.kaiming_normal_(self.ds2[0].weight, 0, 'fan_in', 'relu'); init.constant_(self.ds2[0].bias, 0.0)
        self.nl = CGNL(32, use_scale=False, groups=1)
        ...
        self.convr = nn.Sequential(
            ...
            nn.ConvTranspose3d(14, in_channels, kernel_size=(6, 3, 3), stride=(2, 1, 1), padding=(2, 1, 1), bias=False))
    ...
    def forward(self, inputs):
        ...
        dsfeat1 = self.ds1(msfeat)
        dsfeat2 = self.ds2(dsfeat1) 
        nlout = self.nl(dsfeat2) 
        b0 = self.dfus_block0(nlout)
